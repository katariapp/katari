...
- Shared consequences: category/preference policy, deferred library-update queueing and start, and immediate Entry-refresh
  scheduling
- Operation context: the actual Entry, newly inserted children, favorite state, category membership, previously consumed
  recognized child numbers, and current download preferences
...
| Category and preference policy | The internal `EntryAutomaticDownloadPolicy` is consumed only by the F05 coordinator. Enabled state, favorite state, category inclusion/exclusion, and unread-only filtering remain operation context rather than type capabilities. |
...
- Favorite, categories, preferences, prior consumption, Entry data, and source/runtime outcomes remain contextual.
...
- `spotlessApply`, `git diff --check`, and the build-logic test suite pass.
- Feature Graph tests and API, SPI, Manga, Anime, and Book Entry-interaction compilation pass.
- The combined `checkEntryInteractionBoundaries` run reports 34 expected raw application references owned by F06–F27.
  It reports no F03–F05 download policy or dispatch reference; Library Update retains only its independent F13 Update
  Eligibility migration failure.
- Root Entry-interactions compilation reaches only the three pre-existing F06 Download Lifecycle failures: removed
  `EntryCapabilityReport`, removed `EntryDownloadCapabilityPolicy`, and the resulting runtime generic-inference error.
  The focused F05 test therefore cannot execute until the root main source set is restored by its owning migration.
...
- F07 owns the settings surface and Phase 6 owns general profile/preference context modeling. The current values are
  explicit F05 operation inputs and are not flattened into graph-wide type support.
