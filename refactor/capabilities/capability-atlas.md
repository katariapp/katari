...
| Tracker supported types | Entry tracking action/dialog, Library filters/sorting, auto-add/sync, Stats, settings, backup diagnostics, and documentation | The Tracking Feature/host boundary composes the authoritative registry, actual Entry type, authentication, source acceptance, existing tracks, and tracker sub-capabilities; Entry operations, automatic binding/sync, reconciliation, Migration preparation, accounts/settings/backup, and Library/Stats evidence are migrated | Tracker integrations advertise support independently; current built-in integrations resolve to Manga. Login mechanics and account presentation are tracker-owned declarations, OAuth callbacks remain external mechanics, and raw tracker enforcement closes in Phase 6.7.7. |
