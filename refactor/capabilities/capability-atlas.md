...
| ---- | ----------------- | -------------------------- | ------- |
| Bookmarked download menu availability | Derived application policy | F04 resolves Bulk Downloads plus Bookmarking for the actual selection; the dropdown receives the structured result | Adding a Bookmark provider activates the shared action without a downloader or presentation edit. |
| Missing-child count | Provider-derived presentation | F17 returns the aggregate count with its provider-selected display result | Presentation owns wording only; it contains no Manga applicability gate. |
| Download settings visibility | Independently provider-derived presentation | F07 selects each specialized setting from its real behavior provider | The screen and search consume the same projection and contain no Manga lookup or per-type settings map. |
...
| Source preferences, home page, metering, and WebView contracts | External integration | `ConfigurableSource`, `SourceHomePage`, current `entry.UnmeteredSource`, and WebView source interfaces | Purpose-specific Features compose settings, home, Entry/child WebView, Source Refresh, and F24 queue-warning consequences. Downloader metering stays type-owned; none is a type-wide claim. The unused legacy source-api metering marker is deferred C24 compatibility evidence, not runtime authority. |
...
| Tracker-supported entry types | External integration | `TrackerCapabilities.supportedEntryTypes`, defaulting to Manga | Tracker availability is owned by each integration and must compose with the actual Entry type through the Tracking Feature rather than redefine type support. |
| Tracker dates, privacy, status, deletion, enhanced matching, and scoring | External integration | Tracker capability properties, optional interfaces, and returned option lists | The Tracking Feature projects purpose-specific controls/results; missing sub-capabilities are valid integration differences, not content-type facts or obligations. |
...
| Update eligibility | Library-update worker | Stats calculations and library restriction settings | F13 applies one unconditional smart-update policy to every runtime-contributed content type; no type provider or opt-in exists. Eligible worker entries use the independently unconditional Library Update Refresh Feature, which owns Source Refresh results and passes successful inserted children to F05/F24 flows. |
...
| Local/stub restriction | Entry download, bulk-download, WebView, source-backed actions, Library state, and source presentation | Each owning Feature or presentation request receives operation-scoped source evidence | Context remains consequence-specific; there is no global Local/Stub support capability. |
| Tracker supported types | Entry tracking action/dialog, Library filters/sorting, auto-add/sync, Stats, settings, backup diagnostics, and documentation | The Tracking Feature/host boundary composes the authoritative registry, actual Entry type, authentication, source acceptance, existing tracks, and tracker sub-capabilities; every censused application consequence is migrated and raw tracker access is structurally enforced | Tracker integrations advertise support independently; current built-in integrations resolve to Manga. Login mechanics, account presentation, and OAuth callbacks remain tracker-owned, while consumers receive only neutral Feature models. |
...
| Library-update notification projection | New-update summary, child notifications, and queue-size warning | F24 routes/groups, action sets, and metered-source queue policy; F23 vocabulary; F01 child destination; F09/F04 actions | Book receives explicit Item vocabulary and a derived route. Missing relationships omit only the affected action and preserve notification participation. Library metering is F24-owned; Manga downloader metering remains type-owned F03 mechanics. |
...
| Local source | Direct Manga validation, Manga metadata conversion, and current `SourceMetadata` advertisement in `LocalSource` | The current source contracts consistently describe and produce Manga; returned Entry type remains authoritative. |
| Legacy extension compatibility | `LegacyMangaSourceAdapterTest` verifies Manga metadata/returned types and translation of legacy unmetered traffic policy without losing current source contracts | Strong direct evidence for the documented Manga-only compatibility claim and current-contract translation. |
| Tracking | Synthetic future-service Feature contracts plus genuine tracker synchronization behavior | Feature behavior and contextual blockers are covered without repeating the built-in tracker/type declarations. Current product support remains derived from authoritative tracker registrations. |
...
| Tracking services: Manga only | Current authoritative tracker registrations advertise Manga | Synthetic Feature contracts and genuine synchronization behavior; no copied production matrix | Matched for current integrations and derived at runtime. |
...
| Tracking + entry type | Current trackers support Manga | No current tracker support | No current tracker support | Per-tracker declarations interpreted once by the Tracking Feature. |
