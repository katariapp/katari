...
| Tracker supported types | Entry tracking action/dialog, Library filters/sorting, auto-add/sync, Stats, settings, backup diagnostics, and documentation | The Tracking Feature/host boundary composes the authoritative registry, actual Entry type, authentication, source acceptance, existing tracks, and tracker sub-capabilities; Entry action/session/dialog operations, automatic binding/sync, reconciliation, and Migration preparation are migrated, while account and Library/Stats consumers continue by assigned milestone | Tracker integrations advertise support independently; current built-in integrations resolve to Manga. The host is structurally blocked as an application API, and raw tracker consumers close in Phase 6.7.7 after their assigned migrations. |
