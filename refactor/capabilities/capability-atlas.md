...
| Update eligibility and release-period filtering | F13 synthetic shared-policy contract; synthetic library-filter registry test | Update eligibility is covered as one unconditional behavior contract. F14 release-period filter applicability remains separate and provider-backed. |
...
| Child-list and child-group behavior | F17 graph-selected feature tests, Manga/Anime child-list tests, and synthetic child-group tests | Child List behavior is feature-owned and provider-selected. Book focused behavior and F18 child-group outcomes remain separate coverage work. |
