...
| Tracker supported types | Entry tracking action/dialog, Library filters/sorting, auto-add/sync, Stats, settings, backup diagnostics, and documentation | Tracking Feature composes the authoritative registry, actual Entry type, authentication, source acceptance, existing tracks, and tracker sub-capabilities | Tracker integrations advertise support independently; current built-in integrations resolve to Manga. Raw tracker access is being confined to the tracker system and one host adapter in Phase 6.7. |
