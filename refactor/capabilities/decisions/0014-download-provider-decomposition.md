...
The former `EntryDownloadProcessor` combined core queue and storage behavior with four independently meaningful
capabilities: interactive option resolution, specialized setting behavior, bulk candidate-pool construction, and
automatic-download filtering. Defaults for options and settings made a registered downloader look complete even when it
did not implement those behaviors. Required bulk and automatic-filter methods had the opposite problem: core download
support could not be contributed without also implementing integrations owned by other download workflows.
...
- Core downloading, interactive download options, each specialized download setting, bulk candidate construction, and
  automatic-download filtering are independent provider contracts.
...
- Automatic-download filter providers own only media-specific filtering. Worker orchestration and applicability belong
  to the automatic-download feature integration in Phase 5.
...
- Manga binds core downloads, four setting capabilities, bulk candidate construction, and automatic filtering.
- Anime binds core downloads, interactive options, bulk candidate construction, and automatic filtering.
- Book binds core downloads, bulk candidate construction, and automatic filtering only when its optional downloader is
  constructed. It contributes no false options or settings provider.
- The three type modules no longer register a monolithic download processor separately from their contributions.

## Consequences

- A future type can contribute core downloads without implementing settings, bulk actions, automatic download, or
  interactive options.
- Adding one of those behaviors later is one provider binding and automatically enters graph discovery.
...
- Requiring every core downloader to implement bulk or automatic-download behavior
