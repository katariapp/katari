...
class SyncChapterProgressWithTrackTest {
    ...
    fun `marks remote progress chapters consumed through entry interaction`() = runTest {
        ...
        coEvery {
            entryConsumptionFeature.setConsumed(entry, listOf(chapters[1]), consumed = true)
        } returns EntryConsumptionResult.Changed(listOf(chapters[1]))
    ...
    fun `does not consume chapters when tracker does not support entry type`() = runTest {
        ...
        coVerify(exactly = 0) { entryConsumptionFeature.setConsumed(any(), any(), any()) }
    }
